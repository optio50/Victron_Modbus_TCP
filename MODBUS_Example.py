...
#from pymodbus.utilities import convert_from_registers
...
def modbus_register(address, unit, data_type="int16"):
    msg = client.read_holding_registers(address, count=1, device_id=unit)
    if not msg.isError():
        value = msg.registers[0]  # Extract the first (and only) 16-bit register value
        if data_type == "int16":
            # Convert unsigned 16-bit to signed if value > 32767
            if value > 32767:
                value -= 65536
        # For uint16, no conversion needed
        return value
    else:
        raise AttributeError(f"Modbus read error at address {address}, device_id {unit}")
...
VEbusErrorDict = {
                  0: "No Error",
                  1: "Error 1: Device is switched off because one of the other phases in the system has switched off",
                  ...
                  17: "Error 17: One of the devices assumed master status because the original master failed",
...
while True:
    ...
    try:
        ...
        elif 30 <= BatterySOC < 60:
            BatteryColor = colors.fg.yellow
        else:
            BatteryColor = colors.fg.red
        print(clear, colors.fg.cyan, f" Battery SOC............. ", colors.bold, BatteryColor, f"{BatterySOC:.1f}", " %", colors.reset, sep="")
        ...
        if VEbusError == 0:
            ...
            print(clear, f" VE.Bus Error............ ", colors.fg.red, tr.fill(f"{VEbusErrorDict[VEbusError]}"), "\033[K", sep="")

        # ESS Info
        if ESS_Info.lower() == "y":
            if 1 <= ESSbatteryLifeState <= 8:
            ...
            elif 10 <= ESSbatteryLifeState <= 12:
                print(clear, colors.fg.light_blue, f" ESS SOC Limit (User).... {ESSsocLimitUser:.0f}% Unless Grid Fails", sep="")

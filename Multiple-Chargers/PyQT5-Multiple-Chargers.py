...
# Tested on beta firmware v3.20~35

'''
This file "ONLY" uses MQTT to retreive values.
The MQTT instance ID's must be changed to reflect your system.
...
Victron Grid Tie Inverter in an ESS system: (Such as a Multiplus II)
1 to 3 VE.Direct Victron MPPT's: (Such as Smart Solar Charger MPPT 150/35)
...
'''
#==========================================================
# The number of Smart Solar Chargers (MPPT's)
Solar_Charger_Qty = 3
...
#===================================
'''
MQTT Instance ID & Array Descriptions
Device List in VRM or crossed referenced in the CCGX-Modbus-TCP-register-list.xlsx Tab #2
'''
if Solar_Charger_Qty >= 1:
    MQTT_SolarCharger_1_ID = 279 # <------- Change This
    Array1 = "400W Panel" # New 400W Panel

if Solar_Charger_Qty > 1:
    MQTT_SolarCharger_2_ID = 280 # <------- Change This if you have more than 1
    Array2 = "310W Panel" # New 310W Panel
else:
    MQTT_SolarCharger_2_ID = None
    Array2 = ""

if Solar_Charger_Qty > 2:
    MQTT_SolarCharger_3_ID = 288 # <------- Change This if you have more than 2
    Array3 = "365W Panel" # New 365W Panel
else:
    MQTT_SolarCharger_3_ID = None
    Array3 = ""

MQTT_MultiPlus_ID = 276 # <------- Change This
MQTT_Bmv_ID       = 277 # <------- Change This
MQTT_VEsystem_ID  = 0   # <------- This is left as Zero for MQTT
#===================================
'''
==========================================================
...
# At the very least you must have 1 charger so its included here as topics1
    topics1 = [
...
# If you have 2 or more chargers
    if Solar_Charger_Qty > 1:
        topics2 = [
...
#===================================
    # Uncomment next 2 lines to watch all messages come in via terminal output.
...
#time.sleep(.15) # time to let the keep alive publish resend all values
...
class Window(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        Time_string = None
        # Load the ui file
        uic.loadUi("PyQT5-Multiple-Chargers.ui", self)
        # Set Taskbar Icon
        self.setWindowIcon(QtGui.QIcon('Solar.png'))
        # Tab Widget Start Time
        self.Started_label.setText(f"App Started on {dt_stringStart}")
        # Status Bar Start Time
        self.start_message = QLabel(f"App Started on {dt_stringStart}{big_tab}")
        self.statusBar.addPermanentWidget(self.start_message)
        # Add an Entry to the TextBrowser
        self.textBrowser.append(f"<b><span style=\" color: orangered;\">{dt_stringStart} ---- | ---- App Started<span style=\" color: black;\"</span></b>")
        # Status of the MQTT Connection in StatusBar
        self.statusBar.addPermanentWidget(self.MQTT_label)
        self.statusBar.addPermanentWidget(self.MQTT_LED)
        # Spacer for Statusbar MQTT Message
        self.statusBarLabel = QLabel("    ")
        self.statusBar.addPermanentWidget(self.statusBarLabel)
        if Solar_Charger_Qty == 1:
            self.tabWidget.setTabText(6, "PV 1")
        elif Solar_Charger_Qty == 2:
            self.tabWidget.setTabText(6, "PV 1,2")
        elif Solar_Charger_Qty == 3:
            self.tabWidget.setTabText(6, "PV 1,2,3")

        if Solar_Charger_Qty > 1:
            self.tabWidget.setTabVisible(9, True)
        else:
            self.tabWidget.setTabVisible(9, False)
            for widget in self.Solar_Frame2.findChildren(QWidget):
                widget.setVisible(False)
            #self.Solar_Frame2.setHidden(True)
            self.Solar_Charger2_Error_Value.setHidden(True)
            self.Solar_Charger2_Error_label.setHidden(True)

        if Solar_Charger_Qty > 2:
            self.tabWidget.setTabVisible(10, True)
        else:
            self.tabWidget.setTabVisible(10, False)
            for widget in self.Solar_Frame3.findChildren(QWidget):
                widget.setVisible(False)
            #self.Solar_Frame3.setHidden(True)
            self.Solar_Charger3_Error_Value.setHidden(True)
            self.Solar_Charger3_Error_label.setHidden(True)
...
#  Begin Charts
        ...
        self.watts_connector = DataConnector(watts_plot, max_points=62800, update_rate=2)# max_points=62800, update_rate=2 is almost exact
...
# Chart Battery SOC
        ...
        self.soc_connector = DataConnector(soc_plot, max_points=62800, update_rate=2) # 1.5 seconds in 24 hrs
...
# Chart Battery Volts, Amps, Watts
        ...
        # Data connectors for each plot with dequeue of max_points points
        self.volts_connector     = DataConnector(volts_plot, max_points=62800, update_rate=2) # 1.5 seconds in 24 hrs
        self.bat_watts_connector = DataConnector(bat_watts_plot, max_points=62800, update_rate=2) # 1.5 seconds in 24 hrs
        self.amps_connector      = DataConnector(amps_plot, max_points=62800, update_rate=2) # 1.5 seconds in 24 hrs
...
# Chart Grid Watts, Amps
        ...
        # Data connectors for each plot with dequeue of max_points points
        self.grid_watts_L1_connector = DataConnector(grid_watts_L1_plot, max_points=62800, update_rate=2) # 1.5 seconds in 24 hrs
        self.grid_watts_L2_connector = DataConnector(grid_watts_L2_plot, max_points=62800, update_rate=2) # 1.5 seconds in 24 hrs
        self.grid_amps_L1_connector = DataConnector(grid_amps_L1_plot, max_points=62800, update_rate=2) # 1.5 seconds in 24 hrs
        self.grid_amps_L2_connector = DataConnector(grid_amps_L2_plot, max_points=62800, update_rate=2) # 1.5 seconds in 24 hrs
...
# Chart Cabin Temperatures. Battery Box, Cabin Interior, Cabin Exterior
        ...
        if Analog_Inputs.lower() == 'y':
            ...
            # Data connectors for each plot with dequeue of max_points points
            self.Exterior_Temp_connector = DataConnector(Exterior_Temp_plot, max_points=62800, update_rate=2) # 1.5 seconds in 24 hrs
            self.Interior_Temp_connector = DataConnector(Interior_Temp_plot, max_points=62800, update_rate=2) # 1.5 seconds in 24 hrs
            self.Box_Temp_connector      = DataConnector(Box_Temp_plot,      max_points=62800, update_rate=2) # 1.5 seconds in 24 hrs
...
# Chart A/C Out Watts, Amps
        ...
        # Data connectors for each plot with dequeue of max_points points
        self.ac_out_watts_L1_connector = DataConnector(ac_out_watts_L1_plot, max_points=62800, update_rate=2) # 1.5 seconds in 24 hrs
        self.ac_out_watts_L2_connector = DataConnector(ac_out_watts_L2_plot, max_points=62800, update_rate=2) # 1.5 seconds in 24 hrs
        self.ac_out_amps_L1_connector  = DataConnector(ac_out_amps_L1_plot,  max_points=62800, update_rate=2) # 1.5 seconds in 24 hrs
        self.ac_out_amps_L2_connector  = DataConnector(ac_out_amps_L2_plot,  max_points=62800, update_rate=2) # 1.5 seconds in 24 hrs
        self.ac_out_freq_connector  = DataConnector(ac_out_freq_plot,  max_points=62800, update_rate=2)
...
# Chart Solar Watts PV 1, PV 2, PV 3
        ...
        # Data connectors for each plot with dequeue of max_points points
        self.PV1watts_connector = DataConnector(PV1watts_plot, max_points=62800, update_rate=2)
        if Solar_Charger_Qty > 1:
            PV2watts_plot = LiveLinePlot(pen='cyan',name='PV-2', fillLevel=0, brush=(102,102,255,100))
            # Data connectors for each plot with dequeue of max_points points
            self.PV2watts_connector = DataConnector(PV2watts_plot, max_points=62800, update_rate=2)
        if Solar_Charger_Qty > 2:
            PV3watts_plot = LiveLinePlot(pen='darkgreen',name='PV-3', fillLevel=0, brush=(0,120,50,115))
            # Data connectors for each plot with dequeue of max_points points
            self.PV3watts_connector = DataConnector(PV3watts_plot, max_points=62800, update_rate=2)
...
# Chart Charger #2 State
        ...
        if Solar_Charger_Qty > 1:
            ...
            # Data connectors for each plot with dequeue of max_points points
            self.state2_connector = DataConnector(state2_plot, max_points=14000, update_rate=.2) 
...
# Chart Charger #3 State
        if Solar_Charger_Qty > 2:
            ...
            # Data connectors for each plot with dequeue of max_points points
            self.state3_connector = DataConnector(state3_plot, max_points=14000, update_rate=.2) 
...
#===========================================================================================
        ...
        def Multiplus_Charger():
            ...
            self.textBrowser.append(f"<b>{dt_string} ---- | ---- MultiPlus Manually Changed to Charger Only</b>")
        ...
        def Multiplus_Inverter():
            ...
            mqttpublish.single("W/"+VRMid+"/vebus/"+str(MQTT_MultiPlus_ID)+"/Mode",
                                                        ...
                                                        hostname=ip, port=1883)
            self.textBrowser.append(f"<b>{dt_string} ---- | ---- MultiPlus Manually Changed to Inverter Only</b>")
        ...
        def Multiplus_On():
            ...
            mqttpublish.single("W/"+VRMid+"/vebus/"+str(MQTT_MultiPlus_ID)+"/Mode",
                                                        ...
                                                        hostname=ip, port=1883)
            self.textBrowser.append(f"<b>{dt_string} ---- | ---- MultiPlus Manually Changed to On</b>")
        ...
        def Multiplus_Off():
            ...
            mqttpublish.single("W/"+VRMid+"/vebus/"+str(MQTT_MultiPlus_ID)+"/Mode",
                                                        ...
                                                        hostname=ip, port=1883)
            self.textBrowser.append(f"<b>{dt_string} ---- | ---- MultiPlus Manually Changed to OFF</b>")
        ...
        def ESSbatteryLifeDisabled():
            mqttpublish.single("W/"+VRMid+"/settings/"+str(MQTT_VEsystem_ID)+"/Settings/CGwacs/BatteryLife/State",
                                                        ...
                                                        hostname=ip, port=1883)
            self.textBrowser.append(f"<b>{dt_string} ---- | ---- BatteryLife Manually Disabled</b>")
        ...
        def ESSbatteriesCharged():
            ...
            mqttpublish.single("W/"+VRMid+"/settings/"+str(MQTT_VEsystem_ID)+"/Settings/CGwacs/BatteryLife/State",
                                                        ...
                                                        hostname=ip, port=1883)
            self.textBrowser.append(f"<b>{dt_string} ---- | ---- Keep Batteries Charged Manually Enabled</b>")
        ...
        def Charger1_On():
            mqttpublish.single("W/"+VRMid+"/solarcharger/"+str(MQTT_SolarCharger_1_ID)+"/Mode",
                                                    payload=json.dumps({"value": 1}),
                                                    hostname=ip, port=1883)
            self.textBrowser.append(f"<b>{dt_string} ---- | ---- Charger 1 Manually Changed to ON</b>")
            self.statusBar.showMessage("Charger 1 Manually Changed to ON", 5000)
        ...
        def Charger1_Off():
            mqttpublish.single("W/"+VRMid+"/solarcharger/"+str(MQTT_SolarCharger_1_ID)+"/Mode",
                                                    payload=json.dumps({"value": 4}),
                                                    hostname=ip, port=1883)
            self.textBrowser.append(f"<b>{dt_string} ---- | ---- Charger 1 Manually Changed to OFF</b>")
            self.statusBar.showMessage(f"Charger 1 Manually Changed to OFF", 5000)
        ...
        def Charger2_On():
            mqttpublish.single("W/"+VRMid+"/solarcharger/"+str(MQTT_SolarCharger_2_ID)+"/Mode",
                                                    payload=json.dumps({"value": 1}),
                                                    hostname=ip, port=1883)
            self.textBrowser.append(f"<b>{dt_string} ---- | ---- Charger 2 Manually Changed to ON</b>")
            self.statusBar.showMessage("Charger 2 Manually Changed to ON", 5000)

        def Charger2_Off():
            mqttpublish.single("W/"+VRMid+"/solarcharger/"+str(MQTT_SolarCharger_2_ID)+"/Mode",
                                                    payload=json.dumps({"value": 4}),
                                                    hostname=ip, port=1883)
            self.textBrowser.append(f"<b>{dt_string} ---- | ---- Charger 2 Manually Changed to OFF</b>")
            self.statusBar.showMessage("Charger 2 Manually Changed to OFF", 5000)
        ...
        def Charger3_On():
            mqttpublish.single("W/"+VRMid+"/solarcharger/"+str(MQTT_SolarCharger_3_ID)+"/Mode",
                                                    payload=json.dumps({"value": 1}),
                                                    hostname=ip, port=1883)
            self.textBrowser.append(f"<b>{dt_string} ---- | ---- Charger 3 Manually Changed to ON</b>")
            self.statusBar.showMessage("Charger 3 Manually Changed to ON", 5000)
        ...
        def Charger3_Off():
            mqttpublish.single("W/"+VRMid+"/solarcharger/"+str(MQTT_SolarCharger_3_ID)+"/Mode",
                                                    payload=json.dumps({"value": 4}),
                                                    hostname=ip, port=1883)
            self.textBrowser.append(f"<b>{dt_string} ---- | ---- Charger 3 Manually Changed to OFF</b>")
            self.statusBar.showMessage("Charger 3 Manually Changed to OFF", 5000)
        ...
        def Charger1_Limit():
            ...
            if ok:
                mqttpublish.single("W/"+VRMid+"/solarcharger/"+str(MQTT_SolarCharger_1_ID)+"/Settings/ChargeCurrentLimit",
                                                                   ...
                                                                   hostname=ip, port=1883)
                self.textBrowser.append(f"<b>{dt_string} ---- | ---- Charger 1 Limit Manually Changed to    {answer} Amps</b>")
                self.statusBar.showMessage(f"Charger 1 Limit Manually Changed to {answer} Amps", 5000)
        ...
        def Charger2_Limit():
            ...
            if ok:
                mqttpublish.single("W/"+VRMid+"/solarcharger/"+str(MQTT_SolarCharger_2_ID)+"/Settings/ChargeCurrentLimit",
                                                                   payload=json.dumps({"value": answer}),
                                                                   hostname=ip, port=1883)
                self.textBrowser.append(f"<b>{dt_string} ---- | ---- Charger 2 Limit Manually Changed to    {answer} Amps</b>")
                self.statusBar.showMessage(f"Charger 2 Limit Manually Changed to {answer} Amps", 5000)
        ...
        def Charger3_Limit():
            ...
            if ok:
                ...
                self.textBrowser.append(f"<b>{dt_string} ---- | ---- Charger 3 Limit Manually Changed to    {answer} Amps</b>")
                self.statusBar.showMessage(f"Charger 3 Limit Manually Changed to {answer} Amps", 5000)
        ...
        def GridFeedIn():
            if FeedIn == 1:
                ...
                self.textBrowser.append(f"<b>{dt_string} ---- | ---- Grid Feedin Manually Changed to OFF</b>")
                self.statusBar.showMessage(f"Grid Feedin Manually Changed to OFF", 5000)

            else:
                mqttpublish.single("W/"+VRMid+"/settings/"+str(MQTT_VEsystem_ID)+"/Settings/CGwacs/OvervoltageFeedIn",
                                                                   payload=json.dumps({"value": 1}),
                                                                   hostname=ip, port=1883) # On
                self.textBrowser.append(f"<b>{dt_string} ---- | ---- Grid Feedin Manually Changed to ON</b>")
                self.statusBar.showMessage(f"Grid Feedin Manually Changed to ON", 5000)
        ...
        def SetGridWatts():
            ...
            if ok:
                mqttpublish.single("W/"+VRMid+"/settings/"+str(MQTT_VEsystem_ID)+"/Settings/CGwacs/AcPowerSetPoint",
                                   payload=json.dumps({"value": answer}), hostname=ip, port=1883)
                self.textBrowser.append(f"<b>{dt_string} ---- | ---- Set Grid Watts Manually Changed to    {answer} Watts</b>")
                self.statusBar.showMessage(f"Set Grid Watts Manually Changed to    {answer} Watts", 5000)
        ...
        def VEbusReset():
            ...
            self.statusBar.showMessage(f"Resetting V.E. Bus", 5000)
        ...
        def Input_Amps_Limit():
            ...
            if ok:
                mqttpublish.single("W/"+VRMid+"/vebus/"+str(MQTT_MultiPlus_ID)+"/Ac/ActiveIn/CurrentLimit",
                                                            payload=json.dumps({"value": answer}),
                                                            hostname=ip, port=1883)
                self.textBrowser.append(f"<b>{dt_string} ---- | ---- Grid Input Limit Manually Changed to    {answer} Amps</b>")
                self.statusBar.showMessage(f"Grid Input Limit Manually Changed to    {answer} Amps", 5000)
        ...
        def ESSuser(event):
            ...
            if ok:
                mqttpublish.single("W/"+VRMid+"/settings/"+str(MQTT_VEsystem_ID)+"/Settings/CGwacs/BatteryLife/MinimumSocLimit",
                                                            payload=json.dumps({"value": answer}),
                                                            hostname=ip, port=1883)
                self.textBrowser.append(f"<b>{dt_string} ---- | ---- ESS User Limit Manually Changed to    {answer} Watts</b>")
                self.statusBar.showMessage(f"ESS User Limit Manually Changed to    {answer} Watts", 5000)
        ...
        self.Batt_SOC_progressBar.mouseReleaseEvent = ESSuser # click the progressbar and dialog pops up asking for new ESS user battery limit
        ...
        if Solar_Charger_Qty == 1:
            self.actionCharger_2_Off.setVisible(False)
            self.actionCharger_2_On.setVisible(False)
            self.actionSet_Current_Limit_2.setVisible(False)
            self.actionCharger_3_Off.setVisible(False)
            self.actionCharger_3_On.setVisible(False)
            self.actionSet_Current_Limit_3.setVisible(False)
        if Solar_Charger_Qty == 2:
            self.actionCharger_3_Off.setVisible(False)
            self.actionCharger_3_On.setVisible(False)
            self.actionSet_Current_Limit_3.setVisible(False)
...
#===========================================================================================
    ...
    def update(self): # Update all the UI widgets
        ...
        TotalYield         = SolarYield1
        TotalYieldYest     = SolarYieldYest1
        MaxTotalWattsToday = MaxSolarWatts1
        MaxTotalWattsYest  = MaxSolarWattsYest1
        ...
        if Analog_Inputs.lower() == 'y':
            ...
            if TempSensor1 > Batt_Temp_Lo and TempSensor1 < Batt_Temp_Hi: # Off
                if self.qTimerBatteryBox.isActive():
                    ...
                    self.textBrowser.append(f"<b><span style=\" color: #050a49;\">{dt_string} ---- | ---- Battery Temperature Returned to Normal Level {TempSensor1: .2f} Deg<span style=\" color: black;\"</span></b>")
                ...
                if not self.qTimerBatteryBox.isActive():
                    ...
                    self.textBrowser.append(f"<b><span style=\" color: #6f0f16;\">{dt_string} ---- | ---- Battery Temperature Alert Level {TempSensor1} Deg<span style=\" color: black;\"</span></b>")
# ===========================================================================================
